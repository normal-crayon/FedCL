...
def train(model, train_stream):
    for train_task in train_stream:
        ...
        print(cl_strategy.eval(second_test_stream))
    return len(data)
...
class MnistClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        data = train(model=model, train_stream=second_train_stream)
        return self.get_parameters(), data, {}
    
    def evaluate(self, parameters, config):
        self.set_parameters(parameters)
        data, results = test(model=model, test_stream=second_test_stream)
